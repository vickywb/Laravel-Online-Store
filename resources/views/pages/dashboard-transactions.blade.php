@extends('layouts.dashboard', [
    'headerTitle' => 'Transaction Products',
    'activePage' => 'transaction-products',
    'breadcrumbs' => [
        [
                    ...
                    <div class="row mt-3">
                        <div class="col-12 mt-2">
                         @foreach ($sellTransactions as $transaction)
                            <a class="card card-list d-block" href="{{ route('dashboard-transactions-details', $transaction) }}">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-1">
                                            <img src="{{ $transaction->product->firstImage->file->fileUrl ?? asset('img/blank.png') }}" class="w-50" />
                                        </div>
                                        <div class="col-md-4">
                                        {{ $transaction->product->product_name }}
                                        </div>
                                        <div class="col-md-3">
                                            {{ $transaction->product->user->profile->store_name }}
                                        </div>
                                        <div class="col-md-3">
                                            {{ $transaction->transaction->date_of_transaction->format('j F Y H:i') ?? '' }}
                                        </div>
